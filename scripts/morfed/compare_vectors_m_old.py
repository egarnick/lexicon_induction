...
NUM_TRANS_HYPOS = 50    # Per target word
...
def load_vectors(fvec_file, evec_file):
    global FVEC, EVEC
    for fline in open(fvec_file, 'r'):
        fline_split = fline.strip().split(':')
    ...
    for eline in open(evec_file, 'r'):
        eline_split = eline.strip().split(':')
...
def compare_vectors():
    ...
    for test_tok in TEST_VOCAB:
        test_tok_segs = segment_token(test_tok.lower())
        test_tok_root = morfed_root(test_tok_segs)
        # Only check known words using root from morfessor
        if F_MAP.get(test_tok_root) and FVEC.get(test_tok_root):
            print("Translating root: " + test_tok_root)
            # Break early to test
            if counter == NUM_TRANS_CANDS:
                return
            counter += 1
            candidates = []         # [(translation_cand, sim_score), ...]
            f_feats = FVEC[test_tok_root]
            ...
            for evec in EVEC:
              ...
              # Save full form of word (not morfessor output)
            if len(candidates):
